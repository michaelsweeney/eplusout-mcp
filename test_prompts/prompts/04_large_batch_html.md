...
1. Scan the directory and list all available models. Summarize what building types and climate locations are represented.
2. For each model, retrieve the "End Uses" table. Build a comparison table showing annual Heating (electricity + gas, in GJ) and Cooling (electricity, in GJ) for every model, sorted by total heating load descending.
